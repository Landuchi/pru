...
(function () {
    ...
    var _window = window,
        ...
        _Sys_StringBuilder = Sys.StringBuilder,
        userScope = '',
        ...
        _touch, _host, _odp, _geolocation, _locationWatchId;
    ...
    _app.prototype = {
                    ...
                    if (dataView && !dataView.get_isModal()) {
                        ...
                        if (result) {
                            ...
                            if (survey)
                                ...
                                dataView = _app.showModal(this, controller, view, commandName, commandArgument, appBaseUrl, servicePath, [], { confirmContext: context, showSearchBar: that.get_showSearchBar(), tag: 'discard-changes-prompt-none' });
                            result = false;
                        }
                    }
            }
        ...
        _isActionAvailable: function (action, rowIndex) {
            ...
            if (available) {
                ...
                //if (commandName && commandName.match(/^(Report|Export)/) && that.isDirty())
                //    return false;
            }
        ...
        _createArguments: function (args, view, values) {
            ...
            actionArgs.ExternalFilter = Array.clone(argumentOwner.get_externalFilter());
        ...
        _onExecuteComplete: function (result, context) {
            var that = this,
                ...
                notification, noErrors;

            that._busy(false);
            that._hideWait();

            noErrors = !result.Errors.length;
    ...
    _app.upload = function (method, options) {
        ...
        function addDropText(container, text) {
            $('<span class="app-drop-text">').appendTo(container).text(text);
        }
        ...
        function initialize() {
            if (!container.is('.app-drop-box')) {
                ...
                if (field.OnDemandStyle === 2) {
                    ...
                    $(_touch ? '<span class="material-icon app-icon app-clear">' : '<a class="app-clear ui-btn ui-btn-inline ui-corner-all"/>').insertAfter(container).text(_touch ? 'delete' : resourcesFiles.Clear).attr('title', resourcesFiles.Clear)
    ...
    _app.upload.multi = {
        ...
        send: function (options) {
            ...
            if (index < files.length) {
                ...
                if (options.name)
                    values[options.name] = f.name;
                if (options.type)
                    values[options.type] = f.type;
                if (options.size)
                    values[options.size] = f.size;
                _app.execute({ controller: controllerName, view: options.view, command: 'Insert', values: values }).then(function (result) {
    ...
    $document.on('multifileupload.app', function (e) {
        ...
        if (blobField && files) {
            dataView._externalFilter.forEach(function (fv) {
                ...
                contextValues[fieldName] = dataView.convertStringToFieldValue(dataView.findField(fieldName), fv.Value);
            });
            _touch.pageShown(function () {
                _app.upload.multi.send({
                    ...
                    name: findUtilityField('name', true) || nameField, type: findUtilityField('type', true), size: findUtilityField('size', true), key: controller.key[0].name
                });
            });
    ...
    _app.AccountManager = {
        ...
        avatar: function (user, icon) {
            ...
            if (!avatars) {
                ...
                avatars = this._avatars = userProfileAvatars ? JSON.parse(userProfileAvatars) : {};
            }
            if (avatars)
